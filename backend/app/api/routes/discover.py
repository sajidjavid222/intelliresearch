...
def _parse_deadline(raw: Optional[str]):
    """Best-effort parse of a messy deadline string into a date, else None."""
    import datetime
    import re
...
def _user_topics(user: Optional[User], db_topic_fallback: str = "machine learning") -> list[str]:
    if user and user.research_interests:
        topics = [t.strip() for t in user.research_interests.split(",") if t.strip()]
        if topics:
            return topics[:4]
    return [db_topic_fallback]
...
async def personalized_feed(
    ...
    """A 'For You' feed: recent recommended papers + matching grants + upcoming
    conference/grant deadlines, all keyed off the user's research interests."""
    import asyncio
    import datetime

    topics = _user_topics(user)
    primary = topics[0]
    profile = user.research_interests if user else ""

    papers_task = asyncio.gather(
        *[papers_conn.recommend_papers(t, 4) for t in topics], return_exceptions=True
    )
    grants_task = discovery.grant_agent(primary, profile, 8)
    confs_task = discovery.conference_agent(primary, 10)
    papers_groups, grants, confs = await asyncio.gather(
        papers_task, grants_task, confs_task
    )

    # Flatten + dedupe papers by title, keep most-cited first.
    seen, papers = set(), []
    for grp in papers_groups:
    ...
    # Build a unified upcoming-deadlines timeline from grants + conferences.
    today = datetime.date.today()
    deadlines = []
    for g in grants:
        d = _parse_deadline(g.deadline)
        deadlines.append({
            "kind": "grant", "title": g.title, "org": g.agency,
            "date": d.isoformat() if d else None, "raw": g.deadline,
            "days_left": (d - today).days if d else None, "url": g.url,
        })
    for c in confs:
        d = _parse_deadline(c.submission_deadline)
        deadlines.append({
            "kind": "conference", "title": c.acronym or c.name, "org": c.name,
            "date": d.isoformat() if d else None, "raw": c.submission_deadline,
            "days_left": (d - today).days if d else None, "url": c.url,
        })
    # Upcoming first (future, soonest), then undated/rolling.
    dated = sorted(
        [x for x in deadlines if x["days_left"] is not None and x["days_left"] >= 0],
        key=lambda x: x["days_left"],
    )
    undated = [x for x in deadlines if x["days_left"] is None]

    return {
        "topics": topics,
        "papers": [p.model_dump() for p in papers[:12]],
        "grants": [g.model_dump() for g in grants[:6]],
        "deadlines": (dated + undated)[:12],
    }
